package com.ble.imgtranslator.translator;
...
public class PhoneNumberTranslator extends Translator {
    private static final String TAG = "PhoneNumberTranslator";
...
//        }
        ...
        if (bottom > height)
            bottom = height - 1;
        Log.e(TAG, "after caluc left: " + left + "    top:" + top + "   right:" + right + "     bottom:" + bottom);
        /**
         * 判断区域中有几个字符
         * */
        //已经使用过的像素标记
        ...
        startX = left;
        int charMaxWidth = (right - left) / 4;
        int charMaxHeight = (int) ((right - left) / 4 * 1.5);
        int charWidth = 0;
...
//                    charMaxWidth = charMaxHeight;
        ...
        right = textRect[2];
        bottom = textRect[3];
        Log.e(TAG, "textRect left: " + left + "    top:" + top + "   right:" + right + "     bottom:" + bottom);
...
//        bmp.recycle();
//        // 新建图片
//        Log.e("scantest2", "targetWidth: " + targetWidth + "     targetHeight:" + targetHeight + "    bottom:" + bottom + "   top:" + top);
//        final Bitmap newBmp = Bitmap.createBitmap(Math.abs(targetWidth), Math.abs(targetHeight), Bitmap.Config.ARGB_8888);
//        newBmp.setPixels(targetPixels, 0, Math.abs(targetWidth), 0, 0, Math.abs(targetWidth), Math.abs(targetHeight));
        //将裁切的图片显示出来
        final Bitmap newBmp = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        newBmp.setPixels(usedPixels, 0, width, 0, 0, width, height);
        if (imageView != null) {
            showImage(newBmp);
            Log.e("scantest2", " showImage(newBmp)last ");
        }
    ...
    //修改工卡识别阈值参数参0.5f==>0.6f
    private float proportion = 0.58f;
